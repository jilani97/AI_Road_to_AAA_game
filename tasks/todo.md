...
- [ ] **13.** Difficulty config + skill tree + character roster (foundational — blocks everything else in Phase 2+)
  ...
  - [x] 13-UI: Character-select screen, skill-tree screen, currency HUD, pickup + end-of-run earning hooks. Shipped in 4 slices — 13-UI-1 (currency HUD card + `onCurrencyChange` + end-of-run bonus 25 base + 10/guard + 25 no-damage), 13-UI-2 (9 in-world coin pickups on platforms, spin + bob + proximity collect), 13-UI-3 (pre-run character-select overlay + `CHARACTER_STORAGE_KEY` persistence + `hasStarted` gate on GameApp), 13-UI-4 (skill-tree overlay from pause menu + `SKILLS_STORAGE_KEY` persistence + Hard 4-skill loadout panel + `refreshActiveRunSkills` on unlock/respec/loadout/difficulty change).
...
- [x] **6.** Multiple guards (count scales with difficulty + reinforcements via rooftop hatches, zone-based patrols with semi-random wandering, global alarm state with decay: Normal → Caution → Alert → Evasion)
  ...
  - [x] 6c: Patrol zones + semi-random wandering + count scaling. Pure `guardPatrol.ts` (zones, seeded mulberry32 RNG, `nextPatrolWaypoint` with min-separation retry, `defaultZonesForDifficulty`, `GUARD_COUNT_BY_DIFFICULTY = {easy:2, medium:3, hard:5}`). 17 unit tests. Guards spawn N per difficulty across N authored zones. Per-guard animation polish followed: `GuardAnimationRole` + anim maps moved into the `Guard` interface; every guard loads its own Golem rig asynchronously and animates independently.
...
### Checkpoint 2
- [ ] Full state machine + progression + 3 characters working end-to-end. 10-min playtest with ≥2 near-miss moments documented per character × difficulty. **Remaining gate:** the 10-min playtest itself.
