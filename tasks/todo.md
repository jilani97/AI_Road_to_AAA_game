...
- [ ] **4.** Guard AI state machine (Patrol → Suspicious → Investigating → Alerted → Chasing → Returning) with all per-difficulty scaling: detection timings, hearing radii, give-up timer, guard comms, investigating behaviour, knockout persistence, currency interaction, state indicators (icon + cone colour)
  ...
  - [ ] 4b: Integrate `tickGuardAi` into `GameApp.updateGuard` — drive the existing guard movement/vision-cone colour off `GuardState`, emit noise events from player actions (sprint / dash / landing / attack), track last-known player position, feed patrol-path-close heuristic.
  - [ ] 4c: Guard comms (Easy none / Medium proximity 10 m shout / Hard walkie-talkie) — emerges from multiple guards cooperating; gated on Task 6.
  - [ ] 4d: Knockout model — Easy persistent / Medium 30 s wake / Hard 12 s wake + reinforcements.
